...
runners.filter(function(item) {
  if (
    ...
    runnersLargeSizeShirt.push(`${item.first_name} ${item.last_name}`);
  }
});

console.log(`These runners have large-sized shirts: ${runnersLargeSizeShirt}`);
...
// Problem 1 //Show the last name and how much they donated.
let nameAndDonation = [];
runners.forEach(item =>
  nameAndDonation.push(`${item.last_name}, donated: ${item.donation}`)
);
console.log(nameAndDonation);

// Problem 2 a wealthy doner added $200 to everyone's donation
let updatedDonationsList = runners.map(item => {
  return `Last name: ${
    item.last_name
  }, new donation ${(item.donation += 200)} ||`;
});
console.log(`Here's the updaded donations list: ${updatedDonationsList}`);

// Problem 3 // big doners

let bigDoners = [];
runners.filter(item => {
  if (item.donation >= 350) {
    bigDoners.push(
      `${item.first_name} ${item.last_name} - donation: $${item.donation}  `
    );
  }
});
console.log(`Here's the list of big doners: ${bigDoners}`);
